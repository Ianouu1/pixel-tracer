...
list* create_shape_to_pixel(Shape* shape){
  ...
  case POINT:     pixel_point(shape, lst);break;
  case LINE:      pixel_line(shape, lst);break;
  case SQUAR:     pixel_square(shape, lst);break;
  case RECTANGLE: pixel_rectangle(shape, lst);break;
  case CERCLE:    pixel_cercle(shape, lst);break;
  case POLYGON:   pixel_polygon(shape, lst);break;
  }
...
void pixel_line(Shape* shape, list* lst){
  ...
  px = create_pixel(x, y, shape->color);
  lst_insert_tail(lst, lst_create_lnode(px));
...
void pixel_cercle(Shape* shape, list* lst){
  ...
  Pixel* px;
	...
	while (y >= x) {
    px = create_pixel(p_cercle->center->pos_x + x, p_cercle->center->pos_y + y, shape->color);
    lst_insert_tail(lst, lst_create_lnode(px));
    px = create_pixel(p_cercle->center->pos_x + y, p_cercle->center->pos_y + x, shape->color);
    lst_insert_tail(lst, lst_create_lnode(px));
